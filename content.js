...
(() => {
  ...
  function loadSettings(callback) {
    chrome.storage.sync.get(
      ...
      (res) => {
        settings.hourlyRate = res.hourlyRate && Number(res.hourlyRate) > 0 ?
          Number(res.hourlyRate) :
          DEFAULT.hourlyRate;
        ...
        settings.hoursPerWeek = res.hoursPerWeek && Number(res.hoursPerWeek) > 0 ?
          Number(res.hoursPerWeek) :
          DEFAULT.hoursPerWeek;
        settings.hoursPerDay = res.hoursPerDay && Number(res.hoursPerDay) > 0 ?
          Number(res.hoursPerDay) :
          DEFAULT.hoursPerDay;
        settings.daysPerWeek = res.daysPerWeek && Number(res.daysPerWeek) > 0 ?
          Number(res.daysPerWeek) :
          DEFAULT.daysPerWeek;
  ...
  function formatTimeFromMoney(amount, hourlyRate) {
    if (!isFinite(amount) || amount <= 0 || !isFinite(hourlyRate) || hourlyRate <= 0)
      return null;
    ...
    const UNITS = [{
      short: "c",
      minutes: workMinutesPerYear * 100
    }, {
      short: "y",
      minutes: workMinutesPerYear
    }, {
      short: "mo",
      minutes: workMinutesPerMonth
    }, {
      short: "w",
      minutes: workMinutesPerWeek
    }, {
      short: "d",
      minutes: workMinutesPerDay
    }, {
      short: "h",
      minutes: 60
    }, {
  ...
  function processDocument(root = document.body) {
    ...
    // --- 1️⃣ Process Amazon-style prices first ---
    const amazonPrices = root.querySelectorAll('span.a-price:not([data-mt-processed]), span[aria-hidden="true"]:not([data-mt-processed])');

    amazonPrices.forEach((priceNode) => {
      // ✅ FIX: If the node is inside something we already processed, skip it.
      if (priceNode.closest('[data-mt-processed]')) return;

      const priceWhole = priceNode.querySelector(".a-price-whole");
      const priceFraction = priceNode.querySelector(".a-price-fraction");

      if (!priceWhole) {
        return;
      }

      const wholeStr = priceWhole.textContent.replace(/,/g, "").trim();
      const fractionStr = priceFraction ? priceFraction.textContent.trim() : "0";
      const amount = parseFloat(`${wholeStr}.${fractionStr}`);
      ...
      const clonedPriceNode = priceNode.cloneNode(true);
      clonedPriceNode.removeAttribute("aria-hidden");

      if (settings.displayMode === "tooltip") {
        wrapper.title = timeStr;
        wrapper.appendChild(clonedPriceNode);
      } else {
        wrapper.appendChild(clonedPriceNode);
        const ann = document.createElement("span");
        ann.className = "mt-annotation";
      ...
      if (priceNode.parentNode) {
        priceNode.parentNode.replaceChild(wrapper, priceNode);
      }
    });

    // --- 2️⃣ Process general text nodes ---
  ...
  const debouncedProcess = debounce(runProcessor, 250);
